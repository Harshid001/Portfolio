...
const ParticleMorph = () => {
  ...
  useEffect(() => {
    ...
    const renderer = new THREE.WebGLRenderer({
      alpha: true,
      // A8: this scene is soft alpha-blended point sprites — there are no hard
      // geometry edges for MSAA to resolve, so `antialias: true` was paying for
      // a multisampled backbuffer that changes nothing visible here.
      antialias: false,
    ...
    // A1: tighten the ceiling further on low-tier GPUs once detection resolves.
    // Never raises the ratio above the existing 1.25 cap.
    loadGpuTier().then((tier) => {
      if (disposed) return;
      renderer.setPixelRatio(
        Math.min(window.devicePixelRatio, tier.dprCap, 1.25),
      );
    });
    ...
    const onResize = () => {
      ...
      renderer.setSize(container.clientWidth, container.clientHeight, false);
      // Keep the cached pointer box in sync (defined further down; guarded
      // because resize can fire before the interactivity block runs).
      if (typeof measureBoxRef === "function") measureBoxRef();
    };
    // Late-bound so onResize can reach the pointer-box measurer.
    let measureBoxRef = null;
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++)
      delays[i] = Math.random() * (1 - INTRO_SPREAD);
    ...
    const introStart = prefersReducedMotion
      ? shapes[0].slice()
      : makeJitterStart(shapes[0]);
    ...
    // B3: all four pointer handlers below used to call
    // getBoundingClientRect() per event, forcing a synchronous layout on every
    // mouse/touch move. Cache the box in DOCUMENT space once, refresh it on
    // resize + ResizeObserver, and derive the viewport offset with scroll math
    // so scrolling never invalidates it.
    ...
    measureBox();
    measureBoxRef = measureBox;
    const boxObserver = new ResizeObserver(measureBox);
    boxObserver.observe(container);

    // Normalised device coords from a client point, zero layout reads.
    const ndcX = (clientX) =>
      ((clientX - (box.left - window.scrollX)) / box.width) * 2 - 1;
    const ndcY = (clientY) =>
      -(((clientY - (box.top - window.scrollY)) / box.height) * 2 - 1);
    ...
    const onMouseMove = (e) => {
      mouse.x = ndcX(e.clientX);
      mouse.y = ndcY(e.clientY);
    ...
    const onWheel = (e) => {
      scrollVelocity = Math.max(
        -3,
        Math.min(3, scrollVelocity + e.deltaY * 0.01),
      );
    };
    ...
    const onTouchStart = (e) => {
      if (e.touches.length > 0) {
        isHovering = true;
        targetScale = HOVER_SCALE;
        mouse.x = ndcX(e.touches[0].clientX);
        mouse.y = ndcY(e.touches[0].clientY);
        targetRotation.y = mouse.x * 0.35;
        targetRotation.x = mouse.y * 0.2;
      }
    };
    ...
    const onClick = (e) => {
      _mouseVec.set(ndcX(e.clientX), ndcY(e.clientY));
      ...
      if (t <= 0) return;
      const clickWorldAbs = _raycaster.ray.origin
        .clone()
        .addScaledVector(_raycaster.ray.direction, t);
      const scaleFactor = points.scale.x || 1;
      const clickLocal = clickWorldAbs
        .clone()
        .sub(points.position)
      ...
      if (phase === "hold") {
        const [canvasX, canvasY] = modelSpaceToCanvasPixel(
          clickLocal.x,
          clickLocal.y,
        );
      ...
      if (!prefersReducedMotion) {
        ...
        if (phase === "intro") {
          ...
          for (let i = 0, p = 0; i < restPositions.length; i += 3, p++) {
            const tLocal = Math.min(
              Math.max((introProgress - delays[p]) / INTRO_SPREAD, 0),
              1,
            );
            const e = easeOutCubic(tLocal);
            restPositions[i] =
              introStart[i] + (shapes[0][i] - introStart[i]) * e;
            restPositions[i + 1] =
              introStart[i + 1] + (shapes[0][i + 1] - introStart[i + 1]) * e;
            restPositions[i + 2] =
              introStart[i + 2] + (shapes[0][i + 2] - introStart[i + 2]) * e;
          }
          if (introProgress >= 1) {
            phase = "hold";
            phaseStart = now;
        ...
        // Item 10: _cursorWorld is consumed only by the hover-repel term
        // below (`if (hasCursor)`), so when the pointer is not over the canvas
        // this whole raycast was pure waste on every frame. Skip it.
        let hasCursor = false;
        if (isHovering) {
          ...
          if (Math.abs(denom) > 1e-6) {
            ...
            if (t2 > 0) {
              _cursorWorld
                ...
                .addScaledVector(_raycaster.ray.direction, t2);
              hasCursor = true;
            }
          }
        }
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          boxShadow:
            "0 4px 12px color-mix(in srgb, var(--color-ink) 5%, transparent)",
